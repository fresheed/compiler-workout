...
(*      let compare = compare*)
    ...
    let tostring eprog lbl_proc =
      let rec show_ stmt shift =
        ...
        let print_ext stmt  = (match stmt with
          ...
          (*| ExtSeq _ as es -> Printf.sprintf "%d: %s" (get_label es) (describe es)*)
    ...
    let analyze_vb prog =
      ...
      let kills stmt = match stmt with
        ...
        | ExtRead (_, x) -> map_build_set (SetExpr.empty) (SetExpr.union) (get_dependent_subexprs x) (SetExpr.elements prog_exprs)
        | ExtWrite _ -> SetExpr.empty
        | ExtAssign (_, x, _) -> map_build_set (SetExpr.empty) (SetExpr.union) (get_dependent_subexprs x) (SetExpr.elements prog_exprs)
      ...
      let get_by_label label =
        ...
        let rec find_by_label label stmt =
          ...
          | _ -> None) in
        force_get (find_by_label label prog) in
      let get_successors label = let edges = SetInt2.filter (fun (l_from, _) -> l_from == label) (get_flow prog) in
                                 SetInt.of_list (List.map snd (SetInt2.elements edges)) in
      let rec vb_entry label = let stmt = get_by_label label
                               in SetExpr.union (SetExpr.diff (vb_exit label) (kills stmt)) (gens stmt)
          and vb_exit label = let stmt = get_by_label label
                              in if (is_finish stmt) then SetExpr.empty
                                 else let preds_labels = get_successors label in
...
let describe_vb prog label =
  let on_entry, on_exit = ExtStmt.analyze_vb prog in
  let show_list set = (String.concat ", " (List.map ExtStmt.show_expr (ExtStmt.SetExpr.elements set))) in
